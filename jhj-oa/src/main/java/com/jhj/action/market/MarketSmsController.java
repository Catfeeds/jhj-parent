...
public class MarketSmsController extends BaseController {
	...
	public String sendMarketSms(HttpServletRequest request, Model model, @RequestParam("marketSmsId") Integer marketSmsId) throws InterruptedException,
			ExecutionException {
		...
		if (parameter != null && parameter.equals("99")) {
			String[] mobile = { "18612514665", "18600018345", "18734187116", "15201023689", "15600913197", "18811043684", "13811855734", "13466512812",
					"13488723862", "15011489008", "18405449076", "15010069127" };
		...
		marketSms.setTotalSend(userAll.size());
		marketSmsService.updateByPrimaryKeySelective(marketSms);

		List<Long> sendedUserIds = new ArrayList<Long>();
		List<MarketSmsLog> marketSmsLogList = marketSmsLogService.selectByMarketSmsId(marketSmsId);
		if (!marketSmsLogList.isEmpty()) {
			for (MarketSmsLog item : marketSmsLogList) {
				if (!sendedUserIds.contains(item.getUserId())) {
					sendedUserIds.add(item.getUserId());
				}
			}
		}
		...
		// if(parameter!=null && parameter.equals("99")){
		// String[] mobile=
		// {"18612514665","18600018345","18734187116","15201023689","15600913197","18811043684","13811855734","13466512812","13488723862","15011489008","18405449076","15010069127"};
		//
		// for(int i=0;i<mobile.length;i++){
		// Users u=new Users();
		// Long id=(long) i;
		...
		// List<MarketSmsLog> marketSmsLogList =
		// marketSmsLogService.selectByMarketSmsId(marketSmsId);
		...
		// }
		// if(smsNum==0){
		// sendMarketSmsService.allotSms(u,marketSms.getMarketSmsId()
		// ,smsTempId,content);
		// }
