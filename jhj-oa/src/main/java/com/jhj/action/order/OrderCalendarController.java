...
public class OrderCalendarController extends BaseController {
	...
	public String orderCalender(HttpServletRequest request, Model model, @RequestParam("org_staff_id") Long orgStaffId) {
				...
				if (leaveList != null && leaveList.size() > 0) {
					for (OrgStaffLeave staffLeave : leaveList) {
						...
						if (leaveDate <= weekDateToday && leaveDateEnd >= weekDateToday) {
							...
							String allDay = "";
							String allDayStr = "";
							int start = staffLeave.getStart();
							int end = staffLeave.getEnd();
							if (start == 0 && end == 23) {
								allDay = "allDay";
								allDayStr = "全天";
							}
							...
							eventVo.setAllDay(allDay);
							eventVo.setAllDayStr(allDayStr);
				...
				if (disList != null && disList.size() > 0) {
					for (OrderDispatchs staffDisVo : disList) {
						...
						if (serviceDateStr.equals(weekDate)) {
							...
							String allDay = "";
							String allDayStr = "";
							String serviceDateHourStr =  TimeStampUtil.timeStampToDateStr(serviceDate, "H");
							int serviceDateHour = Integer.valueOf(serviceDateHourStr);
							if (serviceDateHour <= 12) {
								allDay = "am";
								allDayStr = "上午";
							}
