...
public class NewPartnerServiceTypeController extends AdminController {
	...
	public String serviceTypeList(HttpServletRequest request, Model model) {
		if (!model.containsAttribute("contentModel")) {
			...
			List<TreeModel> children = TreeModelExtension.ToTreeModels(partService.getTreeList(), null, null, StringHelper.toIntegerList(expanded, ","));
	...
	public String toAddServiceForm(HttpServletRequest request, Model model, @PathVariable(value = "id") Integer id) {
		...
		String expanded = ServletRequestUtils.getStringParameter(request, "expanded", null);
		if (id != null && id > 0) {
			List<TreeModel> children = TreeModelExtension.ToTreeModels(partService.getTreeList(), id, null, StringHelper.toIntegerList(expanded, ","));
	...
	public String addService(HttpServletRequest request, Model model, @Valid @ModelAttribute("contentModelForm") PartnerServiceType paramServiceType,
			BindingResult result, @PathVariable(value = "id") String id) throws IOException {
	...
	public String edit(HttpServletRequest request, Model model, @PathVariable(value = "id") Long id) {
		...
		if (editModel.getParentId() != null && editModel.getParentId() > 0) {
			List<TreeModel> children = TreeModelExtension.ToTreeModels(partService.getTreeList(), editModel.getParentId().intValue(), null,
	...
	public String editIt(HttpServletRequest request, Model model, @Valid @ModelAttribute("contentModelForm") PartnerServiceType partnerServiceType,
			BindingResult result, @PathVariable(value = "id") Long id) throws JsonParseException, JsonMappingException, IOException {
	 ...
	 * @return
	 */
	@RequestMapping(value = "get-service-types.json", method = RequestMethod.GET)
	public AppResultData<Object> getServiceTypes(@RequestParam(value = "parentId", required = true, defaultValue = "0") Long parentId) {
		...
		list = partService.selectBySearchVo(searchVo);
		result.setData(list);
