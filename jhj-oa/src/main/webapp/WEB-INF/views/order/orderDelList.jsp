                  ...
                  <div class="col-lg-12">
                      <section class="panel">
	                     ...
	                     <header class="panel-heading">
	                        <h4>数据搜索</h4>
                      	 	<form:form class="form-inline" modelAttribute="oaOrderSearchVoModel" action="order-del-list" method="GET">
	                         		<div class="form-group">
	                     					...
	                     					<form:select path="orderStatus" class="form-control">
	                     							...
	                     							<form:option value="6">已取货</form:option>
	                     							<form:option value="7">已送达</form:option>
	                     							<form:option value="9">已关闭</form:option>
	                     					</form:select>
								    </div>
									<div class="form-group">		
										选择云店: <cloudOrgSelectTag:select selectId="${oaOrderSearchVoModel.orgId }"/>
									 </div>
                         ...
                         <!--  <button id="exportExcel" class="btn btn-success">导出Excel</button> -->
                          ...
                          <table class="table table-striped table-advance table-hover" id="table2excel">
                              ...
                              <tr>	
							            ...
							            <td>
							            	${item.disStatusName }
							            	<%-- <c:if test = "${fn:substring(sta.key,0,1)== 1 }">
							            		有效
							            	</c:if>
							            	 <c:if test = "${fn:substring(sta.key,0,1)== 0 }">
							            		无效
							            	</c:if>
							            	<c:if test = "${fn:substring(sta.key,0,1)== 3 }">
							            		暂未派工
							            	</c:if>  --%>
							            </td>
							       	...
							       	<td><button id="btn_update" onClick="btn_update('order/order-del-view?orderNo=${ item.orderNo }&disStatus=${fn:substring(sta.key,0,1) }')" class="btn btn-primary btn-xs" title="订单详情"><i class=" icon-ambulance"></i></button></td>
							       		<%-- <td>
                      ...
                      <c:import url = "../shared/paging.jsp">
	        				...
	        				<c:param name="urlAddress" value="/order/order-del-list"/>
	       			  </c:import>
                  </div>
              </div>
              <!-- page end-->
