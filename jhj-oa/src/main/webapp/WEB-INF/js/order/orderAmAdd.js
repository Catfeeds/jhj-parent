...
function saveFrom() {
	...
	var orderPayType = $("#f-paywawy").val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < serviceType.length; i++) {
				...
				$("#service-content").append(
						"<input type='hidden'value='" + serviceType[i].service_addon_id + "'/>");
				$("#service-content").append("<td>" + serviceType[i].name + "</td>");
				$("#service-content").append(
						"<td>" + serviceType[i].price + "&nbsp;<span>" + serviceType[i].item_unit
								+ "</span></td>");
				$("#service-content").append(
						"<td>" + serviceType[i].dis_price + "&nbsp;<span>"
								+ serviceType[i].item_unit + "</span></td>");
				$("#service-content").append(
						"<td><div><input name='defaultNum' value='" + serviceType[i].default_num
								+ "' onchange='changePrice()'/><div></td>");
				$("#service-content").append("</tr>");
			}
		}
	});
