...
public class OaOrderServiceImpl implements OaOrderService {
	...
	public OaOrderListVo completeVo(Orders orders) {
		...
		if (orders.getOrderStatus().equals(Constants.ORDER_HOUR_STATUS_7) || 
			...
			Long serviceDateDoneTime = (long) (orders.getServiceDate() + orders.getServiceHour() * 3600);
			Long overworkMin = orderDoneTime - serviceDateDoneTime;

			overworkTimeStr = DateUtil.formatDateTime(overworkMin);
	...
	public OaOrderListVo completeNewVo(Orders orders) {
		...
		UserAddrs userAddrs = userAddrService.selectByPrimaryKey(addrId);
		if (userAddrs != null) {
			if (orders.getOrderType() == Constants.ORDER_TYPE_1 || orders.getOrderType() == Constants.ORDER_TYPE_0) {// 钟点工和深度保洁服务地址从userAddr获取
		...
		for (OrderDispatchs item : list) {
			...
			String tmpStaffName = item.getStaffName();
			if (staffName.indexOf(tmpStaffName + ",") < 0)
				staffName += tmpStaffName + ",";

			String tmpStaffMobile = item.getStaffMobile();
			if (staffMobile.indexOf(tmpStaffMobile + ",") < 0)
				staffMobile += tmpStaffMobile + ",";
			...
			if (staffs.getOrgId() != null && staffs.getOrgId() != 0L) {
				Orgs cloudOrg = orgService.selectByPrimaryKey(staffs.getOrgId());
				if (cloudOrg != null) {
					if (cloudName.indexOf(cloudOrg.getOrgName() + ",") < 0)
						cloudName += cloudOrg.getOrgName() + ",";
					vo.setOrgName(cloudOrg.getOrgName());
				}
			}
			...
			Orgs parentOrg = orgService.selectByPrimaryKey(staffs.getParentOrgId());
			if (parentOrg != null) {
				if (orgName.indexOf(parentOrg.getOrgName() + ",") < 0)
					orgName += parentOrg.getOrgName() + ",";
				vo.setParentOrgName(parentOrg.getOrgName());
			}
		...
		if(orders.getValidateCode().equals("1")){
