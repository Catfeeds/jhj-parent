	...
	var orderListSuccess = function(data, textStatus, jqXHR) {
		...
		for (var i = 0; i < orders.length; i++) {
			...
			if(orderStatus == 7 || orderStatus == 8){
				orderRateStylePhone = "none";
				if(order.service_type_name!='初体验基础保洁' && order.service_type_name!='家务订制初体验套餐' && order.service_type_name!='家电清洗'
					&& order.service_type!=69 && order.service_type!=70 && order.service_type!=71 && order.service_type!=73){
					orderagain = "block";
...
function delOrder(orderId, orderStatus){
	...
	if(orderStatus>=1 && orderStatus<7){
		myApp.alert("订单未完成，不能删除",function(){
