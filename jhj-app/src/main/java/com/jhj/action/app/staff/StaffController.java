...
public class StaffController extends BaseController {
	...
	public AppResultData<Object> isWork(HttpServletRequest request, @RequestParam("user_id") Long userId, @RequestParam("is_work") Long isWork,
		...
		Short userType = 1;
		// 向"用户实时地理位置信息表"更新记录
		UserTrailReal userTtailReal = null;
		...
		if (!userTrails.isEmpty()) userTtailReal = userTrails.get(0);
		...
		if (orgStaffBlack != null) {
			result = new AppResultData<Object>(Constants.ERROR_999, ConstantMsg.USER_IN_BLACK, "");
			return result;
		}
		if (isWork == 1) {
			...
			if (orgStaffOnline != null && orgStaffOnline.getIsWork() == 1) {
				result = new AppResultData<Object>(Constants.ERROR_999, ConstantMsg.RESTMONEY_NO_COPY, "");
				return result;
			}
		} else {
			if (orgStaffOnline == null) {
			...
			if (orgStaffOnline != null && orgStaffOnline.getIsWork() == 0) {
				result = new AppResultData<Object>(Constants.ERROR_999, ConstantMsg.GOBACK_NO_COPY, "");
				return result;
			}
		}
	...
	public AppResultData<Object> login(HttpServletRequest request, 
			@RequestParam("mobile") String mobile, @RequestParam("sms_token") String sms_token,
		...
		if ((mobile.trim().equals("18037338893") && sms_token.trim().equals("000000") || mobile.trim().equals("13701187136")
				&& sms_token.trim().equals("000000"))) {
			result = new AppResultData<Object>(Constants.SUCCESS_0, ConstantMsg.SUCCESS_0_MSG, orgStaff);
			return result;
			...
			if (smsToken == null || smsToken.getAddTime() == null || !smsToken.getSmsToken().equals(sms_token)) {
...
//			List<PartnerServiceType> list = partnerServiceTypeService.selectAll();
			...
			orgStaffAuth.setStaffId(orgStaff.getStaffId());
	...
	public AppResultData<Object> getAuth(HttpServletRequest request, @RequestParam("staff_id") Long staffId) {
		...
		if (!StringUtil.isEmpty(orgStaffs.getName()) && !StringUtil.isEmpty(orgStaffs.getCardId())) {
			...
			List<OrgStaffAuth> rs = orgStaffAuthService.selectBySearchVo(searchVo);
	...
	public AppResultData<Object> auth(HttpServletRequest request, @RequestParam("staff_id") Long staffId, @RequestParam("name") String name,
			@RequestParam("card_id") String cardId, @RequestParam(value = "is_hours ", required = false, defaultValue = "0") int isHour,
	...
	public AppResultData<Object> CachPost(@RequestParam("staff_id") Long staffId, @RequestParam("mobile") String mobile) {

		AppResultData<Object> result = new AppResultData<Object>(Constants.SUCCESS_0, ConstantMsg.SUCCESS_0_MSG, "");
		// 1.判断 mobile 对应是否已经有记录 select * from org_staff_invite where
		// invite_mobile = mobile
		// 如果有记录，则返回 “该手机号已经被邀请过”
		// 2.发送短信
		OrgStaffInvite orgStaffInvite = orgStaffInviteService.selectByMobile(mobile);
		if (orgStaffInvite != null) {
			result = new AppResultData<Object>(Constants.ERROR_999, ConstantMsg.STAFF_INVITE, "");
			return result;
		}
		// todo
