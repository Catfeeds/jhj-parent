...
<body>
			...
			<table class="table table-striped table-advance table-hover table2excel" id="table2excel">
				...
				<c:forEach items="${chartDatas.tableDatas}" var="item">
					<tr>
						<th>${item.series}</th>
						<th>
						<c:if test="${item.总单数==0}">
							${item.总单数}
						</c:if>
						<c:if test="${item.总单数>0}">
							<a href="../order/order-list?serviceTypeFlag=1&startTime=${item.startTime}&endTime=${item.endTime}">${item.总单数}</a>
						</c:if>
						</th>
						<th>
	...
	<script type="text/javascript" src="<c:url value='/js/chart/chartAmType.js'/>"></script>
	<script>
