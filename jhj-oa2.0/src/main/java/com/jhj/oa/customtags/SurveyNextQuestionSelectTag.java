 ...
 *
 */
public class SurveyNextQuestionSelectTag extends SimpleTagSupport {
	...
	public SurveyNextQuestionSelectTag() {
	}
    ...
    public void doTag() throws JspException, IOException {
        try {
            ...
            serviceSelect.append("<select id = \"nextQId\" name=\"nextQId\" class=\"form-control\">" );

            if (hasAll.equals("1")) {
            	serviceSelect.append("<option value='0' >请选择下一题</option>");
            }
...
//            SurveyBank item = null;
            ...
            for(int i = 0;  i< list.size();  i++) {
                item = list.get(i);
                selected = "";
                if (selectId != null && item.getqId().equals(selectId)) {
